...
describe('filtrosPorColuna', () => {
  it('coluna texto: separa a string por vírgula em lista de valores', () => {
    const resultado = filtrosPorColuna(
      views,
      { vwia_clientes: ['nome'] },
      { 'vwia_clientes.nome': 'Ana,Bruno' },
    );
    expect(resultado).toEqual({ 'vwia_clientes.nome': { valores: ['Ana', 'Bruno'] } });
  });
  ...
  it('coluna numero: usa min/max a partir de _ini/_fim', () => {
    const resultado = filtrosPorColuna(
      views,
      { vwia_clientes: ['saldo'] },
      { 'vwia_clientes.saldo_ini': '100', 'vwia_clientes.saldo_fim': '500' },
    );
    expect(resultado).toEqual({ 'vwia_clientes.saldo': { min: '100', max: '500' } });
  });

  it('coluna periodo-data: usa ini/fim a partir de _ini/_fim', () => {
    const resultado = filtrosPorColuna(
      views,
      { vwia_clientes: ['data_cadastro'] },
      { 'vwia_clientes.data_cadastro_ini': '2026-01-01' },
    );
    expect(resultado).toEqual({ 'vwia_clientes.data_cadastro': { ini: '2026-01-01' } });
  });

  it('coluna sem valor preenchido não entra no resultado', () => {
    const resultado = filtrosPorColuna(views, { vwia_clientes: ['nome', 'saldo'] }, {});
