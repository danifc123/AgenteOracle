...
_MENSAGEM_BLOQUEADO = (
    "Sua conta foi bloqueada após 3 tentativas de login incorretas. Contate o time de TI pra desbloquear."
)
...
def registrar(mcp) -> None:
    ...
    async def usuarios_route(request: Request, usuario_logado: dict) -> Response:
        ...
        # dar certo ou não.
        chave_rate_limit = f"criar_usuario:{usuario_logado['usuario']}"
        espera = segundos_ate_liberar(chave_rate_limit)
        if espera is not None:
            return _resposta_limite_excedido(
                espera, f"Muitas contas criadas em pouco tempo. Tente de novo em {espera} segundos."
            )
        registrar_falha(chave_rate_limit)
        ...
        if not usuario or not senha or not nome or not papeis_pedidos:
            return JSONResponse(
                {"erro": "Preencha usuário, nome, senha e ao menos um papel."},
                status_code=400,
                headers=CORS_HEADERS,
            )
