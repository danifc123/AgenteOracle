...
def test_espacamento_diferente_gera_mesmo_hash():
    sql_a = "SELECT a, b FROM vwia_titulos_pagar WHERE valor > 100"
    sql_b = "SELECT   a,  b   FROM  vwia_titulos_pagar   WHERE valor > 100"
...
def test_colunas_em_ordem_diferente_geram_mesmo_hash():
    sql_a = "SELECT a, b, c FROM vwia_titulos_pagar"
    sql_b = "SELECT c, a, b FROM vwia_titulos_pagar"
    assert hash_sql(sql_a) == hash_sql(sql_b)


def test_where_diferente_gera_hash_diferente():
    sql_a = "SELECT a FROM vwia_titulos_pagar WHERE valor > 100"
    sql_b = "SELECT a FROM vwia_titulos_pagar WHERE valor > 200"
    assert hash_sql(sql_a) != hash_sql(sql_b)


def test_consulta_com_cte_nao_quebra_mesmo_sem_casar_o_regex():
    sql = "WITH x AS (SELECT a FROM vwia_titulos_pagar) SELECT * FROM x"
